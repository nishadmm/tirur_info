...
<?php
  ...
  $passerr = "";
    ...
    $us = $_SESSION["un"];
    $sql = mysql_query("select password from usertable where username='$us'");
    $sql1 = mysql_query("select user_id from usertable where username='$us'");
    $row2 = mysql_fetch_array($sql);
    $row3 = mysql_fetch_array($sql1);

    $password = $row2["password"];
    $id = $row3["user_id"];
    $enterpassword = $_POST["oldpass"];
    ...
    if($password == $enterpassword) {
      if($newpassword == $renewpassword) {
        ...
        if (strlen($newpassword) >= '8') {
          ...
          if(!preg_match("#[0-9]+#",$newpassword) || !preg_match("#[A-Z]+#",$newpassword) || !preg_match("#[a-z]+#",$newpassword)){
            ...
            if(isset($_POST['change'])) {
              $sql3= "update usertable set password = '".$renewpassword."' where password='".$password."' and user_id='".$id."'";
        ...
        $passerr = "Your New Password Is different in two fields";
      }
    }else {
      $passerr = "Your Current Password Is Wrong";
    }
...
<head>
  ...
  <title>Tirur_Info_Reset_password</title>
</head>
...
<body>
  ...
  <div class="mob-drop-down">
    ...
    <li><a href="#">
          ...
          </div> Re Password <i class="fas fa-angle-down"></i>
        </a>
        <ul>
  ...
  <div class="profile-grid dark-text">
    <div class="side-menu">
      ...
      <a href="profile_resetpassword.php" class="side-selected">Reset Password <i class="fas fa-key"></i></a>
    ...
    <div class="signin-form pass-form">
      <div class="form-container box-shadow">
        <h2 class="center uppercase pbt-1">Change <span class="primary-head"> Password <i class="fas fa-key"></i></span>
        </h2>
        <div class="bottom-line"></div>
        <div class="profile-view">

          <form action="profile_resetpassword.php" method="POST" class=" update-form" id="pass-form"
            name="registration">
            <div class="input-container">
              <label>Current password</label>
              <input class="profile-input" type="password" name="oldpass" required>
            </div>
            <div class="input-container">
              <label>New Password</label>
              <input class="profile-input" type="password" name="newpass" required>
            </div>
            <div class="input-container">
              <label>Re-Enter New Password</label>
              <input class="profile-input" type="password" name="renewpass" required>
              <div class="input-error error update-error">
                <?php echo"$passerr";?>
              </div>
            </div>
            <div class="input-container two-btn">
              <a href="profile_profile.php" class="back-btn btn"><i class="fas fa-angle-double-left"></i> Back</a>
              <input type="submit" name="change" class="update-btn btn" value="Change Password &#10004;">
            </div>
          </form>
        </div>
        <div>
          <p class="lead center mbt-1 form-link">change Shop name & Others ? <a href="profile_update.php"> Click
              Here</a></p>
        </div>
      </div>
    </div>
