...
$msg = "";
  ...
  // messages 
  if($_SESSION["profilesignin"] == true ) {$msg = "Registered Suceessfully..";}
  $_SESSION["profilesignin"] = false;
  
  if($_SESSION["profilelogin"] == true ) {$msg = "Please Login first to see your profile...";}
  $_SESSION["profilelogin"] = false;

  if($_SESSION["updatelogin"] == true ) {$msg = "Please Login first to Update your profile...";}
  $_SESSION["updatelogin"] = false;

  if($_SESSION["resetlogin"] == true ) {$msg = "Please Login first to Reset your Password...";}
  $_SESSION["resetlogin"] = false;

  if($_SESSION["reportlogin"] == true ) {$msg = "Please Login first to Get Report...";}
  $_SESSION["reportlogin"] = false;

  if($_SESSION["logoutlogin"] == true ) {$msg = "You Have No Account To Logout...";}
  $_SESSION["logoutlogin"] = false;

  if($_SESSION["logouted"] == true ) {$msg = "Logout Successfully";}
  $_SESSION["logouted"] = false;
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {  
      ...
      $sql = "select * from usertable where username ='$un' and	password='$pd'";
      ...
      {
        $loginrerr = "The username or password is incorrect..";
      }
  }
  ...
  if($_SESSION["un"]==false)
  {
    ...
    $usr=$_SESSION["un"];
    $user = "<div class='login-show'><span class='primary-head'><i class='fas fa-hat-wizard'></i> Login By : </span>$usr</div>";
  } 
?>
...
<body>
  ...
  <div class="profile-grid dark-text">
    <div class="side-menu">
      ...
      <?php echo $user; ?>
      <a href="profile_login.php" class="side-selected"> <span>Login </span> <i class="fas fa-sign-in-alt"></i></a>
    ...
    <div class="signin-form">
      <div class="uppercase mbt-3" id="login-para">
        <h4>If you want report (information) please login... </h4>
      </div>
      <h2 class="center primary-head pb-1 login-message">
        <?php echo $msg; ?>
      </h2>
      <div class="form-container box-shadow">
        <h2 class="center uppercase pbt-1">user <span class="primary-head"> login <i
              class="fas fa-sign-in-alt"></i></span></h2>
        <div class="bottom-line"></div>
        <p class="center pbt-1 lead signin-lead form-link">Enter your username and password...</p>

        <!-- Form -->
        <form action="profile_login.php" method="POST" id="log-form">
          <div class="input-container">
            <span class="star-error error"></span>
            <label>User Name</label>
            <input class="profile-input" name="user" type="text" required>
          </div>
          <div class="input-container">
            <label>Password</label>
            <span class="star-error error"></span>
            <input class="profile-input" type="password" name="pass" required>
            <div class="input-error error"></div>
          </div>
          <div id="login-error" class="error star-error center">
            <?php echo"$loginrerr";?>
          </div>
          <input type="submit" class="profile-btn" value="LOGIN">
        </form>
        <div>
          <p class="lead center mbt-1 form-link">Don't have an account? <a href="profile_signin.php"> Sign UP</a></p>
        </div>
      </div>
    </div>
    <!-- right side bar -->
    <div class="right-sidebar pbt-3">
      <!-- contact box -->
      <div class="contact-box white-text p-2 box-shadow">
